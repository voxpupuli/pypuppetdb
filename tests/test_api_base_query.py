...
class TestBaseAPIQuery:
    ...
    def test_httperror(self, get, api):
        get.side_effect = requests.exceptions.HTTPError(response=requests.Response())
    ...
    def test_setting_headers_with_token(self, token_api):
        ...
        host_val = request_headers.get("host", request_headers.get("Host"))
        assert host_val == "localhost:8080"
        x_val = request_headers.get(
            "X-Authentication", request_headers.get("x-authentication")
        )
    ...
    def test_with_limit(self, api):
        ...
        assert httpretty.last_request().querystring == {"limit": ["1"]}
        httpretty.disable()
        httpretty.reset()
    ...
    def test_with_include_total(self, api):
        ...
        assert httpretty.last_request().querystring == {"include_total": ["true"]}
        httpretty.disable()
        httpretty.reset()
    ...
    def test_with_offset(self, api):
        ...
        assert httpretty.last_request().querystring == {"offset": ["1"]}
        httpretty.disable()
        httpretty.reset()
    ...
    def test_with_summarize_by(self, api):
        ...
        assert httpretty.last_request().querystring == {"summarize_by": ["1"]}
        httpretty.disable()
        httpretty.reset()
    ...
    def test_with_count_by(self, api):
        ...
        assert httpretty.last_request().querystring == {"count_by": ["1"]}
        httpretty.disable()
        httpretty.reset()
    ...
    def test_with_count_filter(self, api):
        ...
        assert httpretty.last_request().querystring == {"counts_filter": ["1"]}
        httpretty.disable()
        httpretty.reset()
    ...
    def test_with_payload_get(self, api):
        httpretty.enable()
        stub_request("http://localhost:8080/pdb/query/v4/nodes")
        api._query("nodes", payload={"foo": "bar"}, count_by=1)
        assert httpretty.last_request().querystring == {
            "foo": ["bar"],
            "count_by": ["1"],
        }
    ...
    def test_with_payload_post(self, api):
        httpretty.enable()
        stub_request("http://localhost:8080/pdb/query/v4/nodes", method=httpretty.POST)
        api._query("nodes", payload={"foo": "bar"}, count_by=1, request_method="POST")
        assert httpretty.last_request().body == json.dumps(
            {"foo": "bar", "count_by": 1}
        ).encode("latin-1")
        httpretty.disable()
        httpretty.reset()
    ...
    def test_query_bad_request_type(self, api):
        ...
        with pytest.raises(pypuppetdb.errors.APIError):
            api._query(
                "nodes", query='["certname", "=", "node1"]', request_method="DELETE"
            )
        httpretty.disable()
        httpretty.reset()
    ...
    def test_query_with_post(self, api, query):
        ...
        httpretty.enable()
        stub_request("http://localhost:8080/pdb/query/v4/nodes", method=httpretty.POST)
        api._query("nodes", query=query, count_by=1, request_method="POST")
        last_request = httpretty.last_request()
        assert last_request.querystring == {}
        assert last_request.headers["Content-Type"] == "application/json"
        assert last_request.method == "POST"
        assert last_request.body == json.dumps(
            {"query": str(query), "count_by": 1}
        ).encode("latin-1")
        httpretty.disable()
        httpretty.reset()
